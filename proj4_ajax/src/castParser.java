...
public class castParser extends DefaultHandler{
	...
	public void runExample() {
		System.out.println("Begin parsing casts.xml . . .");
		fillAllActors();
		fillAllMovies();
		parseDocument();
		try {
			...
			Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/moviedb?useSSL=false","root", "root1234");
			System.out.println("connected to database");
			
			for(int i = 0; i < add_to_db.size()-1; ++i) {
				Statement s = conn.createStatement();
				String checkpair = "SELECT * FROM stars_in_movies WHERE star_id=" + add_to_db.get(i).getElement1() + " AND movie_id=" + add_to_db.get(i).getElement0();
				ResultSet r = s.executeQuery(checkpair);
				if(!r.next())
					writer.append(add_to_db.get(i).getElement1() + "," + add_to_db.get(i).getElement0() + ",\n");
			}
			
			Statement s = conn.createStatement();
			String checkpair = "SELECT * FROM stars_in_movies WHERE star_id=" + add_to_db.get(add_to_db.size()-1).getElement1() + " AND movie_id=" + add_to_db.get(add_to_db.size()-1).getElement0();
			ResultSet r = s.executeQuery(checkpair);
			if(!r.next())
				writer.append(add_to_db.get(add_to_db.size()-1).getElement1() + "," + add_to_db.get(add_to_db.size()-1).getElement0());
			writer.close();
